...
export default function Hero() {
  ...
  return (
    <div id="home" ref={sectionRef} className="relative w-full min-h-screen overflow-hidden bg-primary bg-fabric-matrix">
      {/* Background Media */}
      <img
      ...
      />
      <div style={{ opacity: sectionVisible ? 1 : 0, transition: 'opacity 800ms ease 100ms' }} className="absolute inset-0 bg-primary/60" />

      {/* Hero Content */}
      <div className="relative z-10 flex flex-col justify-center h-full min-h-screen px-6 md:px-16 lg:px-24 pt-24 pb-24">
        <div className="max-w-xl">
          <h1 style={fadeUp('300ms').style} className="text-4xl md:text-6xl font-extrabold text-white leading-tight mb-2 tracking-tight">Temukan</h1>
          <h1 style={fadeUp('420ms').style} className="text-4xl md:text-6xl font-extrabold text-accent leading-tight mb-4 tracking-tight">Gaya Terbaikmu</h1>
      ...
      {/* Decorative Elements */}
      <div className="hidden md:block absolute right-[5%] top-[10%] text-[160px] opacity-10 pointer-events-none select-none" style={{ opacity: sectionVisible ? 0.1 : 0, transition: 'opacity 1000ms ease 200ms' }}>🪡</div>
      
      {/* Wave divider - Fixed position to be relative so it doesn't overlap next sections */}
      <div 
        style={{
          opacity: sectionVisible ? 1 : 0,
          transition: 'opacity 700ms ease 500ms',
        }}
        className="relative w-full -mt-20 z-0 leading-[0]"
      >
        <svg viewBox="0 0 1440 80" preserveAspectRatio="none" className="w-full h-16 md:h-20 fill-secondary">
