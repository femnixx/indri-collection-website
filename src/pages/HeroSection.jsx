...
function HeroSection() {
    ...
    useEffect(() => {
        ...
        // then we flip to visible — triggering the CSS transitions.
    ...
    // Each element starts hidden/shifted and lands when mounted === true
    ...
    const fadeIn = (delay = '0ms') => ({
        style: { transitionDuration: '900ms', transitionDelay: delay },
        className: `${base} ${mounted ? 'opacity-100' : 'opacity-0'}`,
    });
        ...
        <>
            <div className='relative w-full min-h-screen overflow-hidden'>
                ...
                <div
                    ...
                    className={`absolute inset-0 bg-black/55 ${base} ${mounted ? 'opacity-100' : 'opacity-0'}`}
                />

                {/* Navigation Bar — slides down from top */}
                <div
                    style={{ transitionDuration: '600ms', transitionDelay: '100ms' }}
                    className={`${base} ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-6'}`}
                >
                    <Bar />
                </div>
                ...
                <div className='relative z-10 flex flex-col justify-center h-full min-h-[calc(100vh-80px)] px-6 md:px-16 lg:px-24 pt-8 md:pt-0'>
                    <div className='max-w-xl'>
                        ...
                        {/* Line 2 — slides up, delay 550ms */}
                        <h1
                            className={`${fadeUp('550ms').className} text-4xl md:text-6xl font-extrabold text-cyan-400 leading-tight mb-4`}
                            style={fadeUp('550ms').style}
                        >
                            Perfect Style
                        </h1>

                        {/* Subheading — slides up, delay 700ms */}
                        <p
                        ...
                        >
                            ...
                            <span className='text-cyan-400'>premium fashion pieces</span>{' '}
                            that blend elegance with modern sophistication
                        </p>

                        {/* CTA Button — fades + scales up, delay 900ms */}
                        <div
                            style={{ transitionDuration: '600ms', transitionDelay: '900ms' }}
                            className={`${base} ${mounted ? 'opacity-100 scale-100' : 'opacity-0 scale-90'}`}
                        >
                            <button className='bg-cyan-400 hover:bg-cyan-500 active:scale-95 text-white text-sm md:text-base font-semibold px-7 py-3 rounded-full transition-all duration-200 shadow-lg'>
                                Explore Collection
                            </button>
                        </div>
                ...
                {/* Bottom Wave — slides up, delay 600ms */}
                <div
                ...
                {/* WhatsApp / Chat Floating Button — pops in last, delay 1100ms */}
                <a
                    href='#'
